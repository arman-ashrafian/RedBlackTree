...
class Node:
    ...
    def __repr__(self):
        return '{color} {val}  L: {l_child} R: {r_child}'.format(color=self.color, val=self.value,
                                                            l_child=self.left.value, r_child=self.right.value)
...
class RedBlackTree:
    ...
    def __init__(self):
        ...
        self.root = None
        self.rotations = {
            # Used for deletion and uses the sibling's relationship with parent as a guide to the rotation
    ...
    def _remove_leaf(self, leaf):
        ...
        if leaf.value >= leaf.parent.value:
            # in those weird cases where they're equal
    ...
    def _remove_black_node(self, node):
        ...
        left with is a leaf node which is ready to be deleted
        """
        self.__case_1(node)
        self._remove_leaf(node)
